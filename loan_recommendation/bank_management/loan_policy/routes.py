...
from loan_recommendation.bank_management.loan_policy.services import (
    LoanPolicyService
)
...
print("Template Directory Exists:",
      os.path.exists("loan_recommendation/bank_management/loan_policy/templates"))
print("=" * 80)

templates = Jinja2Templates(
    directory="loan_recommendation/bank_management/loan_policy/templates"
)
...
def loan_policy_list(
    ...
    policies = loan_policy_service.get_bank_policies(
        db=db,
        bank_id=bank_id
    )
...
def add_policy_page(
    ...
    print("=" * 80)
    print(os.getcwd())
    print(os.path.exists("loan_recommendation/bank_management/loan_policy/templates/add.html"))
    print("=" * 80)
...
def create_policy(
    ...
    loan_policy_service.create_loan_policy(
        db=db,
        bank_id=bank_id,
        policy=policy
    )
    ...
    return RedirectResponse(
        url=f"/banks/{bank_id}/policies",
        status_code=303
    )

@router.get("/policies/{policy_id}/edit")
def edit_policy_page(
    policy_id: int,
    request: Request,
    db: Session = Depends(get_db)
):

    policy = loan_policy_service.get_policy(
        db=db,
        policy_id=policy_id
    )
...
def update_policy(
    ...
    loan_policy_service.update_policy(
        db=db,
        policy=policy,
        data=data
    )
    ...
    return RedirectResponse(
        url=f"/banks/{policy.bank_id}/policies",
        status_code=303
    )
